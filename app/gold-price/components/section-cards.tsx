...
export async function SectionCards() {
  ...
  const volatility =
    twentyFourHourChangePercent > 0.1
      ? VOLATILITY.HIGH
      : twentyFourHourChangePercent > 0.05
        ? VOLATILITY.MODERATE
        : VOLATILITY.LOW;
  ...
  return (
    <>
      <div className='hidden md:grid *:data-[slot=card]:from-primary/5 *:data-[slot=card]:to-card dark:*:data-[slot=card]:bg-card grid-cols-1 gap-4 px-4 *:data-[slot=card]:bg-gradient-to-t *:data-[slot=card]:shadow-xs lg:px-6 @xl/main:grid-cols-2 @5xl/main:grid-cols-4'>
        <Card className='@container/card'>
          ...
          <CardFooter className='flex-col items-start gap-1.5 text-sm'>
            ...
            <div className='text-muted-foreground'>Live market price</div>
          </CardFooter>
        </Card>
        <Card className='@container/card'>
          <CardHeader>
          ...
          <CardFooter className='flex-col items-start gap-1.5 text-sm'>
            ...
            <div className='text-muted-foreground'>Volatility: {volatility}</div>
          </CardFooter>
        </Card>
        <Card className='@container/card'>
          <CardHeader>
          ...
          <CardFooter className='flex-col items-start gap-1.5 text-sm'>
            ...
            <div className='text-muted-foreground'>
              Set on{' '}
              {monthlyHighDate?.toLocaleDateString('vi-VN', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
              })}
        ...
        <Card className='@container/card'>
          ...
          <CardFooter className='flex-col items-start gap-1.5 text-sm'>
            ...
            <div className='text-muted-foreground'>
              Set on{' '}
              {monthlyLowDate?.toLocaleDateString('vi-VN', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
              })}
      ...
      <div className='md:hidden px-2 mb-3'>
        <CardDescription>
          Current Gold Price:{' '}
          <span className='font-semibold text-black'>{currentPrice?.toLocaleString('vi-VN')} VND</span>
        </CardDescription>
        <CardDescription>
          24h Change:{' '}
          <span className='font-semibold text-black'>{twentyFourHourChange.toLocaleString('vi-VN')} VND</span>
        </CardDescription>
        <CardDescription>
          Monthly High: <span className='font-semibold text-black'>{monthlyHigh.toLocaleString('vi-VN')} VND</span>
        </CardDescription>
        <CardDescription>
          Monthly Low: <span className='font-semibold text-black'>{monthlyLow.toLocaleString('vi-VN')} VND</span>
        </CardDescription>
      </div>
    </>
  );
